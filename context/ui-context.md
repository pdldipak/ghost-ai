...
A display-only collaborator avatar stack sits at the top-right of the canvas pane, with the Clerk UserButton in the same group. Other participants' cursors appear on the canvas as a colored pointer with a name badge. When presence `isThinking` is true, the badge shows a small spinner; avatars use an `text-ai-text` ring. The AI sidebar shows the latest shared `ai-status` message and locks the Architect composer while generation is active.
