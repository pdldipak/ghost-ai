...
- Trigger.dev setup (`feature-specs/24-trigger-setup.md`): worker runtime `node-22`; `trigger/generate-architecture.ts` product task; pinned `trigger.dev` CLI 4.5.12; `trigger:dev` / `trigger:deploy` scripts; `TRIGGER_SECRET_KEY` documented. No AI jobs, Prisma worker extension, or trigger API routes.
